...
class MapGenerate
{
  ...
  void generateCylinderPointCloud(const double z_resolution,const double theta_num)
  {
    // 先构建轴线为Z轴的圆柱面点云
    int Num = theta_num;
    ...
    pcl::PointCloud<pcl::PointXYZN>::Ptr raw(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    Eigen::Vector3f cylinder_axis(coefficients_cylinder->values[3], coefficients_cylinder->values[4], coefficients_cylinder->values[5]);  // 圆柱体轴线方向向量
    // 保存为txt
